...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
	var that = this;
	/*
	this._send(this._datafeedURL + '/config.txt')
		.done(function(response) {
	...
	var configurationData={
			...
			"supported_resolutions": ["1","3","5","15","30","60","120","360","D","W"]
			}
	that._setupWithConfiguration(configurationData);
};
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
	...
	if (!this._configuration.supports_group_request) {
		...
		var pricescale=100000000;		
		if(priceType=="KRW" || priceType=="KST")  pricescale=2;	
		
		//TODO : 원화마켓에서 소수점 표기시에 pricescale 변경
		
		var data={
			"name": priceType+"|"+coinType,
			"exchange-traded": "COIN",
			"exchange-listed": "bitfriends",
			"timezone": "Asia/Seoul",
			"minmov": 1,
			"minmov2": 0,
			"pointvalue": 1,
			...
			"has_no_volume": false,
			"intraday_multipliers": ["1","3","5","15","30","60","120","360","D","W"],
			"description": priceType+"|"+coinType,
			"type": "coin",
			"supported_resolutions": ["1","3","5","15","30","60","120","360","D","W"],
			"pricescale": pricescale,
			"data_status": "streaming",
			"ticker": priceType+"|"+coinType
		}				
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {	
	...
	//console.log(timeConverter(rangeStartDate), timeConverter( rangeEndDate));
	...
	var historyURL = getHisrotyURL(resolution);	
	/*
	...
	.done(function(response) {		
		...
		if(res==null){
			...
			barsCount =  Object.keys(res).length;			
			Object.keys(res).forEach(function(k){
			    //console.log(k + ' - ' + response[k]['high_price']);
				var dtB = new Date(k.substr(0,4), k.substr(4,2)-1, k.substr(6,2), k.substr(8,2), k.substr(10,2));
				lastChartDate = dtB.getTime();	
				//console.log(k,k.substr(0,4), k.substr(4,2)-1, k.substr(6,2), k.substr(8,2), k.substr(10,2), dtB,dtB.getTime());			
				data.t.push(dtB.getTime()/1000); // times
				data.o.push(res[k]['start_price']*1); // opens
				data.h.push(res[k]['high_price']*1); // highs
				data.l.push(res[k]['low_price']*1); // lows
				data.c.push(res[k]['trd_price']*1); // closes
				data.v.push(res[k]['trd_qty']*1); // volumes
...
Datafeeds.SymbolsStorage = function(datafeed) {
	...
	this._exchangesList = ['bitfriends', coinType];
...
var oldResolution="";
var oldEndDate="";
function getHisrotyURL(resolution){	
	if(oldResolution==resolution && resolution!="D" && resolution!="W"){
		if(endDate>oldEndDate) endDate=getNextDate(1,oldEndDate);	 
		else endDate=getNextDate(-1,oldEndDate);
	}

	if(resolution=="D" || resolution=="W" ){
		resDate=priceType+"/"+coinType+"/"+timeConverter(endDate).substr(0,4);
	}else{
		resDate=priceType+"/"+coinType+"/"+timeConverter(endDate).substr(0,8);	
	}	
	
	var historyURL="";
	if(location.hostname=="127.0.0.1" || location.hostname=="localhost" || location.hostname=="dev.bitfriends.co.kr") historyURL="https://bitfriends-dev.firebaseio.com/chart/";
	else if(location.hostname=="test.bitfriends.co.kr") historyURL="https://bitfriends-prod.firebaseio.com/chart/";
	else if(location.hostname=="prod.bitfriends.co.kr" || location.hostname=="bitfriends.co.kr") historyURL="https://bitfriends-prod.firebaseio.com/chart/";

	if(resolution=="1" || resolution=="0.5") historyURL +="MIN/"+resDate+".json";
	else if(resolution=="3") historyURL += "3MIN/"+resDate+".json";
	else if(resolution=="5") historyURL += "5MIN/"+resDate+".json";
	else if(resolution=="15") historyURL += "15MIN/"+resDate+".json";
	else if(resolution=="30") historyURL += "30MIN/"+resDate+".json";
	else if(resolution=="60") historyURL += "1H/"+resDate+".json";
	else if(resolution=="120") historyURL += "2H/"+resDate+".json";
	else if(resolution=="360") historyURL += "6H/"+resDate+".json";
	else if(resolution=="D") historyURL += "1D/"+resDate+".json";
	else if(resolution=="W") historyURL += "1W/"+resDate+".json";
	
	//이후 URL 변경에 위해
	oldResolution=resolution;
	oldEndDate=endDate;
	//console.log(resolution);
	//console.log(resolution,historyURL);
	return historyURL;
}
